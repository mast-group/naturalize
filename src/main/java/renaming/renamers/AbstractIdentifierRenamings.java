...
public abstract class AbstractIdentifierRenamings implements
	...
	public SortedSet<Renaming> getRenamings(final Scope scope,
		...
		nameAlternatives.add(targetIdentifier); // Give the current identifier a
												// chance...
	...
	public Multiset<NGram<String>> getSnippetNGrams(final String snippet,
		...
		final List<FullToken> lst = checkNotNull(ngramLM).getTokenizer()
				.getTokenListFromCode(snippet.toCharArray());
		...
		for (int i = 0; i < lst.size(); i++) {
			...
			if (fullToken.token.equals(targetIdentifier)
					|| fullToken.token.contains("%" + targetIdentifier + "%")) {
				identifierPositions.add(i);
				sentence.set(i, fullToken.token.replace(targetIdentifier,
						WILDCARD_TOKEN));
			}
		}

		for (final int ngramPos : identifierPositions) {
			for (int i = 0; i < ngramLM.getN(); i++) {
				final int nGramPosition = ngramPos + i;
				if (nGramPosition >= sentence.size()) {
					break;
				}
				final NGram<String> ngram = NGram.constructNgramAt(
						nGramPosition, sentence, ngramLM.getN());
				ngrams.add(ngram);
